...
public class UserEditorGroupsPanelLogic : BaseNetLogic
{
    ...
    private void BuildUIGroups()
    {
        ...
        foreach (var group in groups.Children)
        {
            if (editable.Value)
            {
                var groupCheckBox = InformationModel.MakeObject<Panel>(group.BrowseName, sandbox.ObjectTypes.GroupCheckbox);
            ...
            {
                var groupLabel = InformationModel.MakeObject<Panel>(group.BrowseName, sandbox.ObjectTypes.GroupLabel);
